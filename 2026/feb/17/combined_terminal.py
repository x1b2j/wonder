...
All known issues fixed: Binance incremental depth, Bitfinex trade type error,
Coinbase order book, BitMEX book size, and UI redraw triggers.
"""
...
# Flags to track first depth snapshot for Binance
binance_first_depth = True
...
def binance_on_message(ws, message):
    global binance_status, binance_trades, binance_orderbook, binance_book_updated, binance_first_depth
    try:
        ...
        # Depth (order book) messages
        elif 'e' in data and data['e'] == 'depthUpdate':
            if binance_first_depth:
                # First message is a snapshot – clear and rebuild
                ...
                binance_first_depth = False
            else:
                # Subsequent messages are updates – apply changes
                for bid in data['b']:
                    price = float(bid[0])
                    size  = float(bid[1])
                    if size == 0:
                        binance_orderbook['bids'].pop(price, None)
                    else:
                        binance_orderbook['bids'][price] = size
                for ask in data['a']:
                    price = float(ask[0])
                    size  = float(ask[1])
                    if size == 0:
                        binance_orderbook['asks'].pop(price, None)
                    else:
                        binance_orderbook['asks'][price] = size
            binance_book_updated = True

    except Exception as e:
        binance_trades.append({'text': f"Binance error: {str(e)}", 'side': 'Error'})
...
def binance_on_close(ws, close_status_code, close_msg):
    global binance_status, binance_first_depth
    binance_status = "Disconnected"
    binance_first_depth = True   # reset for next connection

def binance_on_open(ws):
    global binance_status, reconnect_delay, binance_first_depth
...
def bitmex_on_message(ws, message):
    ...
    try:
        ...
        bitmex_trades.append({'text': f"BitMEX error: {str(e)}", 'side': 'Error'})
    ...
    try:
        ...
        if isinstance(data, list) and len(data) >= 2:
            ...
            if len(data) > 2 and data[1] == 'te':
                trade_data = data[2]
                # Ensure trade_data is a list with at least 4 elements
                if isinstance(trade_data, list) and len(trade_data) >= 4:
                    ts = datetime.now(timezone.utc).strftime('%H:%M:%S')
                    amount = trade_data[2]   # positive = buy, negative = sell
                    price  = trade_data[3]
        ...
        bitfinex_trades.append({'text': f"Bitfinex error: {str(e)}", 'side': 'Error'})
...
def coinbase_on_message(ws, message):
    ...
    try:
        ...
        coinbase_trades.append({'text': f"Coinbase error: {str(e)}", 'side': 'Error'})
...
def main(stdscr):
    ...
    # Split each exchange column: give more space to order book (to show up to 10 entries)
    display_height = rect_height - 4
    trade_lines = max(5, display_height // 3)          # at least 5 lines for trades
    book_lines  = display_height - trade_lines         # remainder for order book
