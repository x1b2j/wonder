...
def bitmex_on_message(ws, message):
    ...
    try:
        ...
        elif 'table' in data and data['table'] == 'orderBook10':
            if 'data' in data and len(data['data']) > 0:
                book = data['data'][0]
                
                # Clear and rebuild order book display
                ...
                # Asks (sell orders) - display in reverse (highest first)
                trollbox_messages.append({
                ...
                if 'asks' in book and book['asks']:
                    # Show top 8 asks in reverse order (highest price first)
                    asks = book['asks'][-8:][::-1]
                    for ask in asks:
                        price = ask[0]
                        size = ask[1]
                        trollbox_messages.append({
                            'text': f"${price:>11,.2f}  |  {size:>10,}"
                        })
...
def bitmex_on_open(ws):
    ...
    # Subscribe to trades and order book (top 10 levels)
    subscribe_msg = {
        "op": "subscribe",
        "args": ["trade:XBTUSD", "orderBook10:XBTUSD"]
    }
